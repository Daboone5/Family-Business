/* =========================
        ...
        function getDailySummary(date, hourly) {
            ...
            if (counts.storm >= 3) {
                summary = { icon: "Ã¢â€ºË†Ã¯Â¸Â", label: "Storms Likely" };
            } else if (counts.rain >= 4) {
                summary = { icon: "Ã°Å¸Å’Â§Ã¯Â¸Â", label: "Rain Likely" };
            } else if (counts.storm > 0) {
                summary = { icon: "Ã°Å¸Å’Â¦Ã¯Â¸Â", label: "Storms Possible" };
            } else if (counts.rain > 0) {
                summary = { icon: "Ã°Å¸Å’Â¦Ã¯Â¸Â", label: "Showers Possible" };
            } else if (counts.cloudy > counts.clear) {
                summary = { icon: "Ã¢ËœÂÃ¯Â¸Â", label: "Mostly Cloudy" };
            } else if (counts.cloudy > 0) {
                summary = { icon: "Ã°Å¸Å’Â¤Ã¯Â¸Â", label: "Partly Cloudy" };
            } else {
                summary = { icon: "Ã¢Ëœâ‚¬Ã¯Â¸Â", label: "Sunny" };
            }
        ...
        function summarizePeriod(date, hourly, startHour, endHour) {
            ...
            if (counts.storm > 0) {
                summary = { icon: "Ã¢â€ºË†Ã¯Â¸Â", label: "Storms Possible" };
            } else if (counts.rain > 0) {
                summary = { icon: "Ã°Å¸Å’Â¦Ã¯Â¸Â", label: "Showers Possible" };
            } else if (counts.cloudy > counts.clear) {
                summary = { icon: "Ã¢ËœÂÃ¯Â¸Â", label: "Mostly Cloudy" };
    ...
    try {
        ...
        summaryCard.innerHTML = `
            ...
            <div class="marine-condition-list">
                ...
                <span>Ã°Å¸Å’Â¡Ã¯Â¸Â High ${highTemp}&deg; / Low ${lowTemp}&deg;</span>
                <span>Ã°Å¸Å’Â¬Ã¯Â¸Â Wind ${windDirection} ${windSpeed} mph</span>
                <span>Ã°Å¸Å’Â§Ã¯Â¸Â Rain Chance ${rainChance}%</span>
                <span>Ã¢Ëœâ‚¬Ã¯Â¸Â Peak UV Index: ${uvIndex !== null ? uvIndex : "N/A"} &mdash; ${uvRisk}</span>
                <span>Ã°Å¸Å’â€¢ Moon Phase: ${moonPhase.phase} &mdash; ${moonPhase.illumination}%</span>
                <span>Ã°Å¸Å¡Â¤ Boating Outlook: ${boatingOutlook}</span>
